...
import { allGroupedTabs, isAuthoringTab } from './promptGroups';
...
describe('every prompt in the rail has a hub', () => {
  // Every turn prompt, which is every prompt a hub can be drawn for. The authoring prompts are in the rail
  // but not in the turn: they run outside the pipeline, with no AIRequestType and no request to lay out, so
  // the panel opens them straight into their System template instead of a hub (see SettingsModal).
  const TABS = allGroupedTabs().filter((t) => !isAuthoringTab(t));
