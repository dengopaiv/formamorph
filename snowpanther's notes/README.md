...
| `pod-scripts/` | Two scripts for driving a rented RunPod pod without sitting in an interactive SSH session — `rp.sh` runs one command and returns clean text (the proxy host silently ignores exec'd commands), `pod-setup.sh` brings TabbyAPI up unattended and writes a readable log instead of progress bars. |
